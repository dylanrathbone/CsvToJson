...
class JsonGenerator
  ...
  def generate_json
    ...
    csv_as_array_of_hashes.each_with_index { |current_csv_array_element, current_index|
        ...
        array_of_modifiers = build_array_of_modifier_hashes_from(modifiers_as_hash(current_csv_array_element))

        remove_modifiers_from(temporary_hash_array)

        reformatted_csv_data[current_index] = build_formatted_hash_from(array_of_modifiers, temporary_hash_array)
    }
